...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (state.winner) return res.status(400).json({ error: 'Winner already picked', winner: state.winner });
  if (state.entryPool.length === 0) return res.status(400).json({ error: 'No entries' });
  if (Date.now() < state.roundEndsAt) return res.status(400).json({ error: 'Raffle round not finished' });
...
export async function resetHandler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });
