...
export const PoolHashrateChart = ({ data, blockFoundHistory }) => {
    // Preload images
    const [imagesLoaded, setImagesLoaded] = React.useState(false);
    const pacmanImgRef = React.useRef(null);
    const xmrBlockImgRef = React.useRef(null);
    React.useEffect(() => {
        let loaded = 0;
        ...
        pacmanImg.onload = () => { loaded++; if (loaded === 2)
            setImagesLoaded(true); };
        xmrBlockImg.onload = () => { loaded++; if (loaded === 2)
            setImagesLoaded(true); };
        pacmanImgRef.current = pacmanImg;
        xmrBlockImgRef.current = xmrBlockImg;
    }, []);
    if (!imagesLoaded)
        return _jsx("div", { style: { height: 60 } }); // Don't render until images loaded
    ...
    const pointRadii = displayData.map((_, i) => {
        ...
        if (blockFoundIndices.includes(i))
            return BLOCK_SIZE / 2;
        return 0;
    });
    return (_jsxs("div", { className: "bg-navy/40 rounded-lg p-4 mb-6", children: [_jsx("h4", { className: "text-sm font-bold text-electric/80 mb-2", children: "Pool Hashrate" }), _jsx(Line, { data: {
                    labels,
                    datasets: [
                        {
                            label: 'Pool Hashrate',
