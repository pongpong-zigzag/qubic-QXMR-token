...
function PacmanAnimation({ 
  ...
  const findNearestBlock = (pacmanX: number, pacmanY: number): {x: number, y: number} | null => {
    let nearestBlock: {x: number, y: number} | null = null;
  ...
  const checkBlockCollisions = (pacmanX: number, pacmanY: number) => {
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      if (!animationRunning || !containerRef.current) return;
  ...
  return (
    ...
    >
      {/* XMR Blocks */}
      {blocks.filter(block => !block.eaten && block.visible !== false).slice(-MAX_VISIBLE_BLOCKS).map((block) => (
        <img
          key={block.id}
          src={xmrBlockImg}
          alt="XMR Block"
          className={`absolute transition-opacity duration-300${block.eaten ? ' block-eaten' : ''}`}
          style={{
            left: `${block.x}px`,
            top: `${block.y}px`,
            width: `${block.size}px`,
